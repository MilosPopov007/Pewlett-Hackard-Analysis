...
-- Delete Table (Recreate the retirement_info Table with the emp_no Column )
...
-- Use Left Join for retirement_info and dept_emp tables (we need to create a new table)
SELECT ri.emp_no,
    ri.first_name,
    ri.last_name,
de.to_date
...
-- Employee count by department number (in order), new table, retirement_dept
SELECT COUNT(ce.emp_no), de.dept_no
...
-- New Table  (number of people leaving, List 1. )
SELECT e.emp_no,
    e.first_name,
e.last_name,
    e.gender,
